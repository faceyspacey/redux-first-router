...
export const redirect = (path: string, state: ?Object) => ({ history }) =>
  history.redirect(path, state, false)
...
export const reset = (state: Object | Function, index: ?number, kind: ?string) => ({ history }) =>
  history.reset(state, index, kind, false)
